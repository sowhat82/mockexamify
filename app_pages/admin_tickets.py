...
def show_ticket_response_modal():
    ...
    with st.form("ticket_response_form"):
        ...
        with col1:
            send_response = st.form_submit_button("📤 Send Response", type="primary", use_container_width=True)
...
def show_all_tickets():
    ...
    with col2:
        priority_filter = st.selectbox(
            ...
            key="priority_filter_all"
        )

    with col3:
        category_filter = st.selectbox(
            "Category",
            ["All", "Technical Issue", "Payment & Billing", "Account & Login",
             "Exam Content", "Feature Request", "Bug Report", "Other"],
    ...
    with st.spinner("Loading tickets..."):
        tickets = run_async(load_all_tickets(status_filter, priority_filter, category_filter, sort_by, search_query))
...
def show_ticket_statistics():
    ...
    with col1:
        st.metric(
            "Total Tickets",
            stats.get('total', 0),
            delta=f"+{stats.get('new_today', 0)} today"
        )
    ...
    with col4:
        ...
        st.metric(
            "Resolution Rate",
            f"{resolution_rate}%",
            delta=f"{stats.get('resolution_trend', 0)}%"
        )
...
def display_ticket_card(ticket: Dict[str, Any], show_quick_actions: bool = False, compact: bool = False, key_prefix: str = ""):
    ...
    with st.container():
        ...
        with col2:
            if status not in ['Resolved', 'Closed']:
                if st.button("💬 Respond", key=f"{key_prefix}respond_{ticket_id}", use_container_width=True):
                    show_response_form(ticket)

        with col3:
            if status == "Open" and show_quick_actions:
                if st.button("🚀 Take", key=f"{key_prefix}take_{ticket_id}", use_container_width=True):
                    update_ticket_status(ticket_id, "In Progress")

        with col4:
            if status not in ['Resolved', 'Closed']:
                if st.button("✅ Resolve", key=f"{key_prefix}resolve_{ticket_id}", use_container_width=True):
...
# Database functions
async def load_all_tickets(status: str, priority: str, category: str, sort_by: str, search: str) -> List[Dict[str, Any]]:
...
async def get_ticket_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        new_today = sum(1 for t in tickets if
                       datetime.fromisoformat(t.get('created_at', '').replace('Z', '+00:00')).date() == today)
