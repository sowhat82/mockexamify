...
def show_available_question_pools(user: Dict[str, Any]):
    ...
    st.markdown('<h3 style="color: #000000;">🎯 Take an Exam from Question Pools</h3>', unsafe_allow_html=True)
    st.markdown('<p style="color: #000000;">💡 Questions are randomly selected from pools each time - get a fresh exam every attempt!</p>', unsafe_allow_html=True)
    ...
    try:
        ...
        with col1:
            search_term = st.text_input("🔍 Search pools", placeholder="Search by topic or category...")

        with col2:
            sort_by = st.selectbox("Sort by", ["Newest", "Most Questions", "Name"])
        ...
        st.markdown(f"*Showing {len(filtered_pools)} question pool{'s' if len(filtered_pools) != 1 else ''}*")
...
def show_question_pool_card(pool: Dict[str, Any], container, user: Dict[str, Any]):
    ...
    with container:
        ...
        st.markdown(f"<h3 style='color: #000000; margin-bottom: 0.5rem;'>💼 {pool_name}</h3>", unsafe_allow_html=True)
        st.markdown(f"<p style='color: #666666; margin-top: 0; margin-bottom: 1rem;'>📁 {category}</p>", unsafe_allow_html=True)
        ...
        st.markdown(f"**📝 {total_questions} unique questions available**")
        st.markdown("---")

        # Exam configuration
        st.markdown("**Configure Your Exam:**")
        ...
        if total_questions < 10:
            ...
            question_options = [10, 20, 30, 40, 50, total_questions]
            question_options = [q for q in question_options if q <= total_questions]
        ...
        # Calculate price (1 credit per 20 questions, minimum 1)
        price_credits = max(1, question_count // 20)
        ...
        with col1:
            button_text = f"🚀 Start Exam ({price_credits} credit{'s' if price_credits != 1 else ''})"
            if not can_afford:
...
def filter_pools(pools: List[Dict[str, Any]], search_term: str, sort_by: str) -> List[Dict[str, Any]]:
    ...
    if search_term:
        filtered = [
            ...
            if search_term.lower() in pool.get('pool_name', '').lower() or
               search_term.lower() in pool.get('category', '').lower() or
               search_term.lower() in pool.get('description', '').lower()
        ]
