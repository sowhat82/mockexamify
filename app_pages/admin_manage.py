...
def show_admin_manage():
    ...
    with col1:
        if st.button("⬅️ Back to Dashboard", key="back_to_dashboard_manage"):
    ...
    try:
        ...
        with col4:
            if st.button("📤 Upload New Mock", use_container_width=True):
                st.session_state.page = "admin_upload"
                st.rerun()
        ...
        with col1:
            search_term = st.text_input(
                "🔍 Search mocks", placeholder="Search by title or description..."
            )
        with col2:
...
def show_mock_card(mock: Dict[str, Any]):
    ...
    with st.container():
        ...
        with col1:
            status_badge = "🟢 Active" if mock.get("is_active", True) else "🔴 Inactive"
            explanation_badge = "💡 Explanations" if mock.get("explanation_enabled", False) else ""
        ...
        with col2:
            if st.button(f"✏️ Edit", key=f"edit_{mock['id']}", use_container_width=True):
                st.session_state.edit_mock_id = mock["id"]
                st.session_state.show_edit_form = True

        with col3:
            if st.button(
                f"🗑️ Delete", key=f"delete_{mock['id']}", use_container_width=True, type="secondary"
            ):
        ...
        with col4:
            if st.button(
                f"🤖 Regenerate Explanations", key=f"regen_{mock['id']}", use_container_width=True
            ):
        ...
        with st.expander(f"👀 Preview Questions ({len(questions)} total)"):
            if questions:
...
def show_edit_mock_form(mock_id: str):
    ...
    with st.expander("✏️ Edit Mock Exam", expanded=True):
        # Load mock data
        ...
        with st.form(f"edit_mock_{mock_id}"):
            ...
            with col1:
                ...
                new_price = st.number_input(
                    "Price (Credits)", value=mock.get("price_credits", 1), min_value=1
                )

            with col2:
                new_description = st.text_area("Description", value=mock.get("description", ""))
                new_explanation_enabled = st.checkbox(
                    "Enable Explanations", value=mock.get("explanation_enabled", True)
                )
...
def show_delete_confirmation(mock_id: str):
    ...
    with st.expander("🗑️ Delete Mock Exam", expanded=True):
    ...
    try:
        ...
        for question in questions:
            ...
            explanation = await openrouter_manager.generate_explanation(
                question["question"], question["choices"], question["correct_index"]
            )
...
def filter_mocks(
    mocks: List[Dict[str, Any]], search_term: str, category_filter: str, status_filter: str
) -> List[Dict[str, Any]]:
    ...
    if category_filter != "All":
        filtered = [mock for mock in filtered if mock.get("category", "General") == category_filter]
