...
def show_contact_support():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🎫 Submit Ticket", "❓ FAQ", "📚 Help Center", "🎤 My Tickets"])
...
def show_submit_ticket_form(user: Dict[str, Any]):
    ...
    with st.form("support_ticket_form"):
        ...
        with col2:
            error_message = st.text_input("⚠️ Error Message", placeholder="Any error messages you received")
            affected_exam = st.text_input("📝 Affected Exam", placeholder="Name of exam if issue is exam-specific")
        ...
        if submitted:
            if not subject or not description:
                ...
                if ticket_id:
                    ...
                    st.info("We'll review your ticket and respond within 24 hours. You can track its status in the 'My Tickets' tab.")
                    st.balloons()
                else:
...
def show_faq_section():
    ...
    st.info("Can't find what you're looking for? Submit a support ticket and we'll help you personally!")
...
def show_ticket_card(ticket: Dict[str, Any]):
    ...
    with st.container():
        ...
        with col1:
            if st.button("👁️ View Details", key=f"view_ticket_{ticket_id}", use_container_width=True):
                show_ticket_details(ticket)
        
        with col2:
            if status not in ['Resolved', 'Closed']:
                if st.button("💬 Add Reply", key=f"reply_ticket_{ticket_id}", use_container_width=True):
                    show_ticket_reply_form(ticket)
        
        with col3:
            if status not in ['Closed']:
                if st.button("❌ Close Ticket", key=f"close_ticket_{ticket_id}", use_container_width=True):
