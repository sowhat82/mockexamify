...
def show_production_monitoring():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["🏥 Health", "⚡ Performance", "💾 Cache", "📋 Logs", "🔔 Alerts"]
    )
...
def render_status_overview(health_status: Dict[str, Any]):
    ...
    status_color = (
        ...
        else "#ffc107" if overall_status == "degraded" else "#dc3545"
    )
    status_icon = (
    ...
    with col4:
        ...
        memory_color = (
            "#28a745" if memory_usage < 70 else "#ffc107" if memory_usage < 85 else "#dc3545"
        )
        st.markdown(
            f"""
...
def render_health_monitoring(health_status: Dict[str, Any]):
    ...
    if system_stats:
        ...
        with col1:
            ...
            st.metric(
                "CPU Usage", f"{cpu_percent:.1f}%", delta="High" if cpu_percent > 80 else "Normal"
            )
        ...
        with col3:
            ...
            st.metric(
                "Disk Usage", f"{disk_usage:.1f}%", delta="High" if disk_usage > 90 else "Normal"
            )
...
def render_performance_monitoring(production_metrics: Dict[str, Any]):
    ...
    fig.add_trace(
        go.Bar(x=functions, y=avg_times, name="Avg Time (ms)", marker_color="skyblue"), row=1, col=1
    )
...
def render_cache_monitoring(cache_stats: Dict[str, Any]):
    ...
    with col2:
        ...
        fig.update_layout(
            title="Cache Hit Rate Trend", xaxis_title="Time", yaxis_title="Hit Rate (%)", height=300
        )
...
def render_log_monitoring():
    ...
    if selected_logger != "All":
        filtered_logs = [
            ...
            if log["logger"] == selected_logger.lower().replace(" ", "_")
        ]
    if log_level != "All":
    ...
    with col2:
        fig = px.pie(df, values="Count", names="Level", title="Log Level Distribution")
        fig.update_layout(height=300)
        st.plotly_chart(fig, use_container_width=True)
...
def render_alerts_monitoring():
    ...
    for alert in current_alerts:
        ...
        severity_icon = {"CRITICAL": "🚨", "WARNING": "⚠️", "INFO": "ℹ️", "SUCCESS": "✅"}.get(
            alert["severity"], "📢"
        )
    ...
    fig.add_trace(go.Bar(x=dates, y=critical_counts, name="Critical", marker_color="#dc3545"))

    fig.add_trace(go.Bar(x=dates, y=warning_counts, name="Warning", marker_color="#ffc107"))

    fig.add_trace(go.Bar(x=dates, y=info_counts, name="Info", marker_color="#007bff"))
