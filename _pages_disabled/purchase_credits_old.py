...
def display_package_card(pack_id: str, pack_info: Dict[str, Any], user: Dict[str, Any]):
    ...
    if st.button(
        f"💳 Purchase {name}", key=f"purchase_{pack_id}", use_container_width=True, type=button_type
    ):
...
def initiate_stripe_checkout(pack_id: str, pack_info: Dict[str, Any], user: Dict[str, Any]):
    ...
    try:
        with st.spinner("Creating secure checkout session..."):
            ...
            checkout_url = run_async(
                stripe_manager.create_checkout_session(
                    user=user, pack_id=pack_id, success_url=success_url, cancel_url=cancel_url
                )
            )
        ...
        if status == "success":
            ...
            try:
                ...
                if refreshed_user:
                    ...
                    st.success(
                        f"✅ Updated balance: {refreshed_user.get('credits_balance', 0)} credits"
                    )
            except:
