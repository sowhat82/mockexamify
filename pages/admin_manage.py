...
def show_admin_manage():
    ...
    try:
        ...
        with col1:
            search_term = st.text_input("🔍 Search mocks", placeholder="Search by title or description...")
        with col2:
...
def show_mock_card(mock: Dict[str, Any]):
    ...
    with st.container():
        ...
        with col3:
            if st.button(f"🗑️ Delete", key=f"delete_{mock['id']}", use_container_width=True, type="secondary"):
        ...
        with col4:
            if st.button(f"🤖 Regenerate Explanations", key=f"regen_{mock['id']}", use_container_width=True):
