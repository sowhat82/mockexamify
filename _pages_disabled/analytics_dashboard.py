...
from models import ExamCategoryCode, DifficultyLevel
...
def show_analytics_dashboard():
    ...
    analytics_data = asyncio.run(load_comprehensive_analytics(user.get('id'), exam_category, time_range))
...
async def load_comprehensive_analytics(user_id: str, exam_category: str, time_range: str) -> Dict[str, Any]:
    ...
    try:
        ...
        filtered_attempts = [
            ...
            if attempt.created_at >= start_date
        ]
...
def calculate_performance_metrics(attempts: List) -> Dict[str, Any]:
    ...
    return {
        ...
        'average_time_per_question': (np.mean(time_spent) / len(attempts[0].answers)) if time_spent and attempts else 0,
...
def analyze_study_patterns(attempts: List) -> Dict[str, Any]:
    ...
    return {
        ...
        'favorite_study_days': [day for day, count in 
                               sorted([(day, study_days.count(day)) for day in set(study_days)], 
...
def calculate_comparative_analysis(attempts: List, mastery_data: List) -> Dict[str, Any]:
    ...
    user_avg_score = np.mean([
        (sum(1 for ans in attempt.answers if ans.get('is_correct', False)) / len(attempt.answers)) * 100
        for attempt in attempts if attempt.answers
    ])
    ...
    return {
        ...
        'benchmark_comparison': {
            level: {'difference': user_avg_score - benchmark, 'achieved': user_avg_score >= benchmark}
...
def generate_advanced_recommendations(attempts: List, mastery_data: List) -> Dict[str, Any]:
    ...
    for attempt in sorted(attempts, key=lambda x: x.created_at, reverse=True)[:10]:
        if attempt.answers:
            score = (sum(1 for ans in attempt.answers if ans.get('is_correct', False)) / len(attempt.answers)) * 100
    ...
    if weak_topics:
        ...
        recommendations['immediate_actions'].append(f"Prioritize studying: {', '.join(weak_topics[:2])}")
...
def calculate_improvement_potential(user_score: float, user_level: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'weekly_improvement_target': improvement_needed / 8 if improvement_needed > 0 else 0  # Assume 8 weeks
    }
...
def render_analytics_overview(analytics_data: Dict[str, Any]):
    ...
    with col5:
        st.metric(
            "Consistency",
            f"{metrics['consistency_score']:.0f}/100",
    ...
    if metrics['improvement_trend'] == 'strong_improvement':
        ...
        insights.append("📊 Your performance is stable. Consider challenging yourself with harder questions.")
    ...
    if progress_data['milestones_achieved']:
        ...
        for i, (milestone, attempt_num) in enumerate(achieved_milestones.items()):
            with cols[i % len(cols)]:
                st.success(f"{milestone_names.get(milestone, milestone)}\nAchieved on attempt #{attempt_num + 1}")
    else:
...
def render_topic_analysis(analytics_data: Dict[str, Any]):
    ...
    weak_topics = [(topic, data) for topic, data in topic_analytics.items() 
                   if data['mastery_level'] < 60]
    strong_topics = [(topic, data) for topic, data in topic_analytics.items() 
    ...
    with col1:
        ...
        if weak_topics:
            for topic, data in sorted(weak_topics, key=lambda x: x[1]['mastery_level'])[:5]:
                st.warning(f"**{topic}**: {data['mastery_level']:.1f}% mastery - {data['attempts']} attempts")
        else:
    ...
    with col2:
        ...
        if strong_topics:
            for topic, data in sorted(strong_topics, key=lambda x: x[1]['mastery_level'], reverse=True)[:5]:
...
def render_study_patterns(analytics_data: Dict[str, Any]):
    ...
    if not session_data.get('no_data'):
        ...
        with col2:
            ...
            session_counts = [session_data['short_sessions'], session_data['medium_sessions'], session_data['long_sessions']]
            
            fig = px.pie(
                values=session_counts,
                names=session_lengths,
    ...
    if study_patterns['study_frequency_per_week'] < 3:
        recommendations.append("📈 Increase study frequency to 3-4 sessions per week for better retention")
    ...
    if study_patterns['study_streak_current'] == 0:
        ...
        recommendations.append(f"🎯 Extend your streak to 7 days! Currently at {study_patterns['study_streak_current']}")
    
    if session_data and not session_data.get('no_data'):
        if session_data['average_minutes'] < 10:
            recommendations.append("⏱️ Consider longer study sessions (15-25 minutes) for deeper learning")
...
def render_progress_reports(analytics_data: Dict[str, Any], exam_category: str):
    ...
    if recommendations:
        ...
        st.info(f"**Difficulty Level**: {recommendations.get('difficulty_adjustment', 'Continue current level')}")
        st.info(f"**Practice Frequency**: {recommendations.get('practice_frequency', 'Maintain current frequency')}")
    ...
    with col2:
        if st.button("📈 Export Data"):
            ...
            summary_data = {
                ...
                'progress_summary': progress_data if not progress_data.get('insufficient_data') else {},
