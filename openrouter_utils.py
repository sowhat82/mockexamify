...
class OpenRouterManager:
    ...
    def _detect_and_fix_typos(self, question_text: str, choices: List[str]) -> Dict[str, Any]:
        ...
        TYPO_PATTERNS = [
            (r"\bajoint\b", "a joint"),
            (r"\bhusbandimmediately\b", "husband immediately"),
            (r"\btobe\b", "to be"),
            (r"\bofthe\b", "of the"),
            (r"\binthe\b", "in the"),
            (r"\bonthe\b", "on the"),
    ...
    async def fix_question_errors(
        ...
        try:
            ...
            logger.info(f"AI fix_result has_changes: {fix_result.get('has_changes', False)}")
            logger.info(f"AI changes_made: {fix_result.get('changes_made', {})}")
            logger.info(f"Typo fixes has_changes: {typo_fixes['has_changes']}")
            logger.info(f"Typo changes_made: {typo_fixes['changes_made']}")
...
Be thorough in your reasoning. If unsure, lower your confidence score."""
    ...
    def _parse_fix_errors_response(
        ...
        try:
            ...
            logger.error(f"Raw AI response (first 500 chars): {response[:500]}")

        # Fallback: return original content unchanged
        logger.warning("Using fallback - returning original content unchanged")
        return {
            "fixed_question": original_question,
            "fixed_choices": original_choices,
            "changes_made": {"question": [], "choices": {}},
