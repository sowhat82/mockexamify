...
try:
    ...
    OCR_AVAILABLE = True
    logger.info("OCR support enabled (EasyOCR + PyMuPDF)")
except ImportError as e:
    logger.warning(f"OCR not available: {e}. Install easyocr and pymupdf for scanned PDF support.")
...
class DocumentParser:
    ...
    async def _parse_pdf(self, file_content: bytes, file_name: str) -> ParseResult:
        """Parse PDF file for MCQ content, with OCR fallback for scanned documents"""
        try:
            ...
            if len(clean_text) > 100:  # Got reasonable amount of text
                logger.info(
                    f"PDF {file_name}: Extracted {len(clean_text)} chars via standard method"
                )
                return await self._extract_mcqs_from_text(text, file_name)

            # Text extraction failed or minimal text - try OCR
            logger.info(
                f"PDF {file_name}: Minimal text extracted ({len(clean_text)} chars), attempting OCR..."
            )
            ...
            if not OCR_AVAILABLE:
                return ParseResult(
                    ...
                    questions=[],
                    errors=[
                        "PDF appears to be scanned/image-based. OCR libraries not available. Install easyocr and pdf2image, or use a text-based PDF."
                    ],
                    total_parsed=0,
                    confidence_score=0.0,
                )
            ...
            # Use OCR
            ocr_text = await self._ocr_pdf(file_content, file_name)
            if ocr_text and len(ocr_text.strip()) > 100:
                logger.info(f"PDF {file_name}: OCR extracted {len(ocr_text)} chars")
                return await self._extract_mcqs_from_text(ocr_text, file_name)
            else:
                return ParseResult(
                    success=False,
                    questions=[],
                    errors=["Could not extract text from PDF via standard extraction or OCR."],
                    total_parsed=0,
                    confidence_score=0.0,
                )
    ...
    async def _ocr_pdf(self, file_content: bytes, file_name: str) -> str:
        ...
        try:
            ...
            for i, page in enumerate(pdf_doc):
                logger.info(f"Processing page {i+1}/{num_pages} with OCR...")
            ...
            combined_text = "\n\n".join(all_text)
            logger.info(
                f"OCR complete: extracted {len(combined_text)} chars from {num_pages} pages"
            )
    ...
    async def _parse_csv(self, file_content: bytes, file_name: str) -> ParseResult:
        ...
        try:
            ...
            confidence_score = (
                0.9
                if not errors
                else max(0.5, 1.0 - len(errors) / len(questions)) if questions else 0.0
            )
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                return ParseResult(
                    ...
                    questions=[],
                    errors=[
                        "Invalid JSON format: expected list of questions or object with 'questions' key"
                    ],
                    total_parsed=0,
                    confidence_score=0.0,
                )
            ...
            confidence_score = (
                0.95
                if not errors
                else max(0.7, 1.0 - len(errors) / len(questions)) if questions else 0.0
            )
    ...
    def _parse_text_block(self, block: str, block_num: int) -> Optional[ParsedQuestion]:
        ...
        try:
            ...
            # Clean and join stem
            stem = (
                " ".join(stem_lines[1:]) if len(stem_lines) > 1 else stem_lines[0]
    ...
    def _parse_json_question(
        self, item: Dict[str, Any], question_num: int
...
async def process_syllabus_document(
    file_content: bytes, file_type: str, file_name: str
) -> Tuple[List[str], List[str]]:
    """Process syllabus document into chunks"""
