...
class StripeUtils:
    ...
    def create_checkout_session(
        self, package_key: str, user_id: str, user_email: str, success_url: str, cancel_url: str
        ...
        try:
            print(
                f"[DEBUG] create_checkout_session called with package_key={package_key}, user_id={user_id}, user_email={user_email}, success_url={success_url}, cancel_url={cancel_url}"
            )
            if package_key not in self.credit_packages:
                print(f"[DEBUG] Invalid credit package selected: {package_key}")
                return False, None, "Invalid credit package selected"
            package = self.credit_packages[package_key]
            print(f"[DEBUG] Creating Stripe session for package: {package}")
                ...
                line_items=[
                    {
                        "price_data": {
                            ...
                            "product_data": {
                                ...
                                "images": ["https://wantamock.streamlit.app/logo.png"],
                            },
            ...
            print(f"[DEBUG] Stripe session created: {session.url}")
            return True, session.url, None
        except stripe.StripeError as e:
            logger.error(f"Stripe error creating session: {e}")
            print(f"[DEBUG] Stripe error creating session: {e}")
            return False, None, f"Payment error: {str(e)}"
        except Exception as e:
            logger.error(f"Unexpected error creating checkout session: {e}")
            print(f"[DEBUG] Unexpected error creating checkout session: {e}")
    ...
    async def process_successful_payment(self, session_data: Dict) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            st.info(f"[DEBUG] user_id value: {user_id}, type: {type(user_id)}")
            print(f"[DEBUG] user_id value: {user_id}, type: {type(user_id)}")
            try:
                uuid_obj = uuid.UUID(str(user_id))
            except Exception as e:
                st.error(f"[ERROR] user_id is not a valid UUID: {user_id}")
                print(f"[ERROR] user_id is not a valid UUID: {user_id}")
                return False, "Invalid user_id (not a UUID)"

            st.info(
                f"[DEBUG] Attempting db.create_payment with: user_id={user_id}, session_id={session_id}, amount={amount_paid / 100}, credits={credits_to_add}"
            )
            print(
                f"[DEBUG] Attempting db.create_payment with: user_id={user_id}, session_id={session_id}, amount={amount_paid / 100}, credits={credits_to_add}"
            )
            ...
            existing_payment = await db.get_payment_by_session_id(session_id)
            st.info(f"[DEBUG] Existing payment for session_id: {existing_payment}")
            print(f"[DEBUG] Existing payment for session_id: {existing_payment}")
            if existing_payment:
                st.info("[DEBUG] Payment already exists for this session_id. Skipping insert.")
                print("[DEBUG] Payment already exists for this session_id. Skipping insert.")
                return True, None
            ...
            st.info(f"[DEBUG] db.create_payment result: {payment_record}")
            print(f"[DEBUG] db.create_payment result: {payment_record}")
            if not payment_record:
                # Try to get error from Supabase result if available
                try:
                    from db import db as db_module

                    result = (
                        db_module.client.table("payments")
                        .select("*")
                        .eq("stripe_session_id", session_id)
                        .execute()
                    )
                    st.info(f"[DEBUG] Supabase select result after failed insert: {result}")
                    print(f"[DEBUG] Supabase select result after failed insert: {result}")
                    if hasattr(result, "error") and result.error:
                        st.info(f"[DEBUG] Supabase error: {result.error}")
                        print(f"[DEBUG] Supabase error: {result.error}")
                except Exception as e:
                    st.info(f"[DEBUG] Exception while checking Supabase error: {e}")
                    print(f"[DEBUG] Exception while checking Supabase error: {e}")
    ...
    def calculate_value_savings(self, package_key: str) -> Optional[str]:
        ...
        base_price_per_credit = (
            self.credit_packages["starter"]["price"] / self.credit_packages["starter"]["credits"]
        )
        actual_price_per_credit = package["price"] / package["credits"]

        if actual_price_per_credit < base_price_per_credit:
            savings_percent = (
                (base_price_per_credit - actual_price_per_credit) / base_price_per_credit
            ) * 100
...
def create_payment_button(
    ...
    if success and session_url:
        ...
        st.markdown(
            f'<meta http-equiv="refresh" content="0; url={session_url}">', unsafe_allow_html=True
        )
        return True
    else:
