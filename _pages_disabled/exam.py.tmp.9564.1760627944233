...
def reset_exam_state():
    ...
    keys_to_reset = [
        'exam_state', 'current_mock', 'answers', 'current_question',
        'start_time', 'exam_id', 'questions', 'time_limit',
        'flagged_questions', 'exam_results'
    ]
...
def generate_exam_pdf_download(user: Dict[str, Any], results: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        # Generate PDF
        filepath, download_url = run_async(generate_exam_results_pdf(
            attempt_data, mock_data, user_data, include_explanations=True
        ))
