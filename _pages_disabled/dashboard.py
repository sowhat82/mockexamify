...
def show_available_question_pools(user: Dict[str, Any]):
    ...
    st.info(
        "💡 Questions are randomly selected from pools each time - get a fresh exam every attempt!"
    )
    ...
    try:
        ...
        with col1:
            search_term = st.text_input(
                "🔍 Search pools", placeholder="Search by topic or category..."
            )
        ...
        st.markdown(
            f"*Showing {len(filtered_pools)} question pool{'s' if len(filtered_pools) != 1 else ''}*"
        )
