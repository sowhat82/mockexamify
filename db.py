...
class DatabaseManager:
    ...
    # ========================================================================
    # HitPay Payment Methods (PayNow) - Separate from Stripe
    # ========================================================================

    async def create_hitpay_payment(
        self,
        user_id: str,
        hitpay_payment_id: str,
        amount: float,
        credits_purchased: int,
        status: str = "pending",
    ) -> Optional[Dict]:
        """Create a HitPay payment record (PayNow)"""
        try:
            # Check if this is a demo user
            is_demo_user = user_id in ["student-demo-id", "admin-demo-id", "demo-user-id"]

            if self.demo_mode or is_demo_user:
                logger.info(f"Creating mock HitPay payment for demo user: {user_id}")
                return {
                    "id": f"demo-hitpay-{len(DEMO_ATTEMPTS)}",
                    "user_id": user_id,
            ...
            if not result.data:
                logger.error(f"Failed to record HitPay payment: {result}")
            return result.data[0] if result.data else None
        except Exception as e:
            logger.error(f"Error creating HitPay payment: {e}", exc_info=True)
            return None

    async def get_payment_by_hitpay_id(self, hitpay_payment_id: str) -> Optional[Dict]:
        """Get payment by HitPay payment ID"""
        try:
            if self.demo_mode:
                logger.info(f"Demo mode: HitPay payment check for {hitpay_payment_id}")
                return None
            ...
            result = (
                self.admin_client.table("payments")
                .select("*")
                .eq("hitpay_payment_id", hitpay_payment_id)
                .execute()
            )
            return result.data[0] if result.data else None
        except Exception as e:
            logger.error(f"Error getting payment by HitPay ID: {e}")
            return None
