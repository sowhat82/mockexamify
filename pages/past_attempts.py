...
def show_performance_overview(attempts: List[Dict[str, Any]]):
    ...
    recent_avg = sum(attempt.get('score', 0) for attempt in recent_attempts) / len(recent_attempts) if recent_attempts else 0
    ...
    with col1:
        st.metric(
            "Total Attempts", 
            total_attempts,
            help="Total number of mock exams completed"
        )
    ...
    with col3:
        st.metric(
            "Best Score", 
            f"{best_score:.1f}%",
            help="Your highest score achieved"
        )
    ...
    with col4:
        st.metric(
            "Pass Rate", 
            f"{pass_rate:.1f}%",
            help="Percentage of exams passed (≥60%)"
        )
...
def apply_filters(attempts: List[Dict[str, Any]], date_filter: str, score_filter: str, sort_order: str) -> List[Dict[str, Any]]:
    ...
    if date_filter != "All Time":
        ...
        filtered = [
            attempt for attempt in filtered
            if attempt.get('created_at') and 
            datetime.fromisoformat(attempt['created_at'].replace('Z', '+00:00')) >= cutoff
        ]
    ...
    with st.container():
        ...
        with col3:
            ...
            if explanations_unlocked:
                ...
                if st.button("🔓 Unlock Explanations", key=f"unlock_{index}", use_container_width=True):
    ...
    with col2:
        if current_credits >= cost:
            if st.button("🔓 Unlock Now", use_container_width=True, type="primary"):
                success = run_async(unlock_explanations_for_attempt(attempt['id'], user['id'], cost))
                if success:
