        ...
        if st.button("🚀 Process All Files", type="primary"):
            if uploaded_files:
                process_uploaded_files(
                    uploaded_files, exam_category, auto_tag, auto_difficulty, create_variants
                )
            else:
...
def process_uploaded_files(
    files, exam_category: str, auto_tag: bool, auto_difficulty: bool, create_variants: bool
):
        ...
        try:
            ...
            if uploaded_file.type == "application/pdf":
                results = process_pdf_file(
                    file_content, upload.id, exam_category, auto_tag, auto_difficulty
                )
            elif (
                uploaded_file.type
            ...
            asyncio.run(
                db.update_upload_status(
                    upload.id, ProcessingStatus.COMPLETED, extraction_results=json.dumps(results)
                )
            )

            all_results.append({"filename": uploaded_file.name, "results": results})
...
def process_pdf_file(
    file_content: bytes, upload_id: int, exam_category: str, auto_tag: bool, auto_difficulty: bool
) -> Dict[str, Any]:
    ...
    try:
        ...
        questions = asyncio.run(
            ai.extract_questions_from_text(parsed_data["content"], exam_category, auto_tag=auto_tag)
        )
...
def process_docx_file(file_content: bytes, upload_id: int, exam_category: str) -> Dict[str, Any]:
    ...
    try:
        ...
        syllabus_data = syllabus_processor.process_document(
            parsed_data["content"], f"upload_{upload_id}.docx"
        )
        ...
        return {
            ...
            "success_rate": (
                len(saved_sections) / len(syllabus_data.get("sections", []))
                if syllabus_data.get("sections")
                else 0
            ),
        ...
        for idx, row in df.iterrows():
            try:
                ...
                question_data = {
                    ...
                    "topic_tags": (
                        str(row.get("topics", "")).split(",") if row.get("topics") else []
                    ),
                }
                ...
                if auto_difficulty:
                    ...
                    question_data["difficulty"] = difficulty_analysis.get(
                ...
                if auto_tag:
                    topic_tags = asyncio.run(
                        ai.tag_question_topics(
                            question_data["question"], question_data["choices"], exam_category
                        )
                    )
...
def render_syllabus_management_tab():
    ...
    with col1:
        ...
        try:
            docs = asyncio.run(
                db.get_syllabus_docs_by_category(ExamCategoryCode(selected_category))
            )
    ...
    with col2:
        ...
        with st.form("add_syllabus_section"):
            ...
            section_type = st.selectbox(
                "Section Type",
                ["introduction", "core_content", "regulations", "case_studies", "appendix"],
            )
        ...
        try:
            questions = asyncio.run(
                ai.generate_syllabus_coverage_questions(
                    doc.title, doc.learning_objectives, doc.exam_category.value, num_questions=10
                )
            )
...
def render_ai_training_tab():
    ...
    with col1:
        ...
        exam_category = st.selectbox(
            "Exam Category", [category.value for category in ExamCategoryCode]
        )
        ...
        if generation_type == "Question Set":
            ...
            if st.button("🎯 Generate Question Set", type="primary"):
                generate_ai_question_set(
                    topic, difficulty, num_questions, exam_category, include_scenarios
                )

        elif generation_type == "Question Variants":
            # Select base question
            questions = asyncio.run(
                db.get_questions_by_category(ExamCategoryCode(exam_category), limit=20)
            )

            if questions:
                question_options = {f"Q{q.id}: {q.question_text[:100]}...": q.id for q in questions}
                selected_q_text = st.selectbox(
                    "Select Base Question", list(question_options.keys())
                )
            ...
            syllabus_docs = asyncio.run(
                db.get_syllabus_docs_by_category(ExamCategoryCode(exam_category))
            )

            if syllabus_docs:
                doc_options = {f"{doc.title}": doc.id for doc in syllabus_docs}
                selected_doc_text = st.selectbox(
                    "Select Syllabus Section", list(doc_options.keys())
                )
...
def generate_ai_question_set(
    topic: str, difficulty: str, num_questions: int, exam_category: str, include_scenarios: bool
):
...
def generate_adaptive_practice(
    weak_areas: List[str], difficulty: str, exam_category: str, num_questions: int
):
...
def generate_topic_coverage(syllabus_doc_id: int, questions_per_objective: int, exam_category: str):
    ...
    with st.spinner("Generating topic coverage questions..."):
        try:
            ...
            questions = asyncio.run(
                ai.generate_syllabus_coverage_questions(
                    doc.title, doc.learning_objectives, exam_category, total_questions
                )
            )
        ...
        try:
            ...
            for i, question in enumerate(questions):
                try:
                    ...
                    tags = asyncio.run(
                        ai.tag_question_topics(
                            question.question_text, question.choices, exam_category
                        )
                    )
        ...
        try:
            ...
            for i, question in enumerate(questions):
                try:
                    ...
                    analysis = asyncio.run(
                        ai.analyze_question_difficulty(
                            {
                    ...
                    suggested_difficulty = analysis.get(
                        "suggested_difficulty", question.difficulty.value
                    )
            ...
            if difficulty_changes:
                ...
                for change in difficulty_changes[:10]:  # Show first 10
                    with st.expander(
                        f"Q{change['question_id']}: {change['current']} → {change['suggested']}"
                    ):
                        st.write(change["explanation"])

                        if st.button(f"Apply Change", key=f"apply_{change['question_id']}"):
                            asyncio.run(
                                db.update_question_difficulty(
                                    change["question_id"], DifficultyLevel(change["suggested"])
                                )
                            )
...
def enhance_all_explanations(exam_category: str):
    ...
    with st.spinner("Enhancing explanations..."):
        try:
            questions = asyncio.run(
                db.get_questions_with_basic_explanations(ExamCategoryCode(exam_category))
            )
            ...
            for i, question in enumerate(questions):
                try:
                    ...
                    if enhanced_explanation != question.explanation_seed:
                        # Update explanation
                        asyncio.run(
                            db.update_question_explanation(question.id, enhanced_explanation)
                        )
                        enhanced_count += 1
